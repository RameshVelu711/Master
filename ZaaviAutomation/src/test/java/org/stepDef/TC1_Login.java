...
public class TC1_Login extends BaseClass {
	...
	public void userPerformSearchOfProduct(String prod) {
		...
		pom.getLoginPage().getBtnSearchSendkeys().sendKeys(prod,Keys.ENTER);
		
	}


	@Then("User verify the searched items are displayed {string}")
	public void userVerifyTheSearchedItemsAreDisplayed(String expValue) throws InterruptedException {
		
		String actValue = pom.getLoginPage().getTxtVerify().getText();
		
		Assert.assertEquals("verify after success of search message",expValue,"TV & Movie Graphic T-Shirts");
